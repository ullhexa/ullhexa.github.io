...
import {createFloorTransitions} from './floor-transitions.js?v=59';
...
export function createScenery(map,svg){
 ...
 async function render(darkness,floors={}){
  ...
  const ticket=++request;if(ready&&nextArt===artKey&&nextLight===lightKey&&nextFloor===floorKey)return;
  try{
  ...
  }catch{if(ticket===request){floorTransitions.clear();ready=false;plane.hidden=true;scene.style.visibility='';}}
 }
