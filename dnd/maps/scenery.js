...
import {createFloorTransitions} from './floor-transitions.js?v=81';
...
export function createScenery(map,svg,onStatus=()=>{}){
 ...
 for(const id of ['artwork','floor-layers','terrain-layers','roof-layers','lighting-layer'])scene.append(document.getElementById(id));svg.before(scene);scene.style.visibility='hidden';
 ...
 if(map.userMap){
  const url=scene.querySelector('#artwork image').getAttribute('data-src'),darkness=document.createElement('div');darkness.className='scenery-darkness';plane.append(darkness);let transform='',source=null,request=0,ready=false;
  return {async render(level){darkness.style.opacity=level/100;const ticket=++request;if(!ready)onStatus('loading');try{source??=loadRaster(url).catch(error=>{source=null;throw error;});const image=await source;if(ticket!==request)return;if(!ready){image.className='scenery-original';image.alt='';plane.prepend(image);ready=true;plane.hidden=false;}onStatus('ready');}catch(error){if(ticket===request)onStatus('error',error);}},position({scale,x,y}){const next=`translate3d(${x}px,${y}px,0) scale(${scale})`;if(next!==transform){plane.style.transform=next;transform=next;}}};
 }
 ...
 const floorTransitions=createFloorTransitions(map,plane,art,light);
 const images=[...scene.querySelectorAll('image')],sources=new Map();let transform='',artKey='',lightKey='',floorKey='',request=0,ready=false;
 ...
 async function render(darkness,floors={}){
  light.style.opacity=darkness/100;
  const visible=images.filter(painted),nextArt=visible.map(n=>`${n.getAttribute('data-src')}:${n.getAttribute('clip-path')||''}`).join('|'),nextLight=[...scene.querySelectorAll('#night-darkness-mask>g,#night-lights>g')].map(n=>n.style.display).join('|');
  const nextFloor=JSON.stringify(floors);
  const ticket=++request;if(ready&&nextArt===artKey&&nextLight===lightKey&&nextFloor===floorKey){onStatus('ready');return;}
  onStatus('loading');
  try{
   // SVG nodes retain inert source references: only painted layers start requests.
   const rasters=await Promise.all(visible.map(n=>load(n.getAttribute('data-src'))));if(ticket!==request)return;
   ...
   floorTransitions.update(floors,ready);floorKey=nextFloor;
   ...
   ready=true;plane.hidden=false;onStatus('ready');
  }catch(error){if(ticket===request){floorTransitions.clear();onStatus('error',error);}}
 }
