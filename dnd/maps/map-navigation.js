...
export function createMapNavigation({map,getCamera,viewport,setCamera,prepare}){
 const control=button('',null,'toolbar-icon map-joystick');control.id='map-joystick';control.setAttribute('aria-label','Drag to move map');control.append(icon([{d:'M12 2v5m-3-2 3-3 3 3M12 22v-5m-3 2 3 3 3-3M2 12h5m-2-3-3 3 3 3M22 12h-5m2-3 3 3-3 3',fill:'none',stroke:'currentColor','stroke-width':1.5,'stroke-linejoin':'round'},{tag:'circle',cx:12,cy:12,r:3,fill:'currentColor',class:'joystick-knob'}]));document.getElementById('open-dice').after(control);let drag=null,lastStep=null;
