import { STORY_SCENES } from './story-scenes.js?v=25';
...
export function validProject(value) {
  return !!value&&value.version===1&&['battle','story'].includes(value.mode)&&list(value.maps)&&value.maps.length>0&&list(value.stories)&&value.stories.length>0&&value.stories.every(id=>ids.includes(id))&&value.stories.includes(value.vibe)&&validSceneGroups(value,'maps')&&validSceneGroups(value,'stories');
}
