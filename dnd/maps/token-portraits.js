...
export function tokenPortrait(member){if(typeof member==='number')member={portrait:member};const key=member.avatar||`${member.item?'item':member.monster?'monster':'player'}:${member.portrait}`;
  if(!faces.has(key))faces.set(key,(async()=>{const a=member.avatar?null:member.item?itemAsset(member.portrait):portraitAsset(member.portrait,member.monster),image=await load(a?a.url:await assetURL(member.avatar));
    ...
    const canvas=document.createElement('canvas');canvas.width=canvas.height=TOKEN_FACE_SIZE;const c=canvas.getContext('2d');c.imageSmoothingEnabled=true;c.imageSmoothingQuality='high';if(member.item){const ratio=(TOKEN_FACE_SIZE-8)/Math.max(w,h);c.drawImage(image,crop?crop[0]*image.naturalWidth:0,crop?crop[1]*image.naturalHeight:0,w,h,(TOKEN_FACE_SIZE-w*ratio)/2,(TOKEN_FACE_SIZE-h*ratio)/2,w*ratio,h*ratio);}else c.drawImage(image,x,y,size,size,0,0,TOKEN_FACE_SIZE,TOKEN_FACE_SIZE);return canvas.toDataURL('image/png');})());return faces.get(key);
}
...
export function setFace(img,member){const key=member.avatar||`${member.item?'item':member.monster?'monster':'player'}:${member.portrait}`;if(img.dataset.face===key)return;img.dataset.face=key;tokenPortrait(member).then(url=>{if(img.dataset.face===key)img.src=url;}).catch(()=>{img.removeAttribute('src');});}
