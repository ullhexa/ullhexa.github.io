...
import {normalizeFeatures,featureEnabled} from './board-state.js?v=62';
import {itemOnSelectedFloor} from './floors.js?v=62';
import {normalizeItems, normalizeMembers, normalizeCampaign, syncCampaign, fiveFeet, initiativeOrder} from './combat-state.js?v=97';
...
export function activePartyName(state){
 const name=state.public?state.partyName:state.campaign?.parties.find(p=>p.id===state.campaign.activeParty)?.name;
 return typeof name==='string'?name.trim().slice(0,48)||'Party':'Party';
}
...
export function normalizeEncounter(map,input,party) {
  ...
  return {roster,itemSchema:2,items:input.itemSchema===2?normalizeItems(input.items):[],monsters:normalizeMembers(input.monsters,true),...(input.public?{public:true,placeMarkers:placeMarkersVisible(input),itemBorders:itemBordersVisible(input),partyName:activePartyName(input),features:normalizeFeatures(input.features)}:{campaign:normalizeCampaign(input.campaign,roster)}),tokenMode:input.tokenMode==='players'?'players':'party',regroupPlayers:typeof input.regroupPlayers==='boolean'?input.regroupPlayers:input.tokenMode!=='players',shapes};
}
...
export function playerProjection(state) {
  ...
  const publicMember=m=>{const {hp,maxHp,hpLinked,tempHp,ac,statCard,statText,spells,...safe}=m;return safe;};
  return {...rest,placeMarkers:placeMarkersVisible(state),itemBorders:itemBordersVisible(state),partyName:activePartyName(state),features:normalizeFeatures(state.public?state.features:campaign?.features),public:true,turnId:state.public?state.turnId:initiativeOrder(state).some(m=>m.id===state.turnId)?state.turnId:initiativeOrder(state)[0]?.id||null,roster:(featureEnabled(state,'party')?state.roster:[]).map(publicMember),items:(featureEnabled(state,'items')?state.items||[]:[]).filter(m=>m.visible&&itemOnSelectedFloor(m,state)).map(({comment,templateId,...safe})=>safe),monsters:(featureEnabled(state,'encounter')?state.monsters||[]:[]).filter(m=>m.visible).map(m=>({...publicMember(m),initiative:null})),shapes:state.shapes.filter(s=>s.visible!==false).map(s=>({...s,visible:true}))};
}
