...
export function drawDie(canvas,shape,value,angles,settled){
 ...
 for(const f of faces){const points=f.points.map(project),light=Math.round(27+Math.max(0,dot(f.normal,unit([-.5,1,2])))*28);ctx.beginPath();points.forEach(([x,y],i)=>i?ctx.lineTo(x,y):ctx.moveTo(x,y));ctx.closePath();ctx.fillStyle=`hsl(148 18% ${light}% / .7)`;ctx.fill();ctx.strokeStyle='#e9cea0';ctx.lineWidth=1.4;ctx.lineJoin='round';ctx.stroke();}
 if(settled){const front=faces.find(f=>f.index===0)||faces.at(-1),p=project(front.center);ctx.fillStyle='#fff8e9';ctx.strokeStyle='#14241c';ctx.lineWidth=2;ctx.font=`700 ${String(value).length>1?27:32}px system-ui`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.translate(p[0],p[1]+1);ctx.scale(.85,1);ctx.strokeText(String(value),0,0);ctx.fillText(String(value),0,0);}
}
