...
const projectVertex=v=>[52+v[0]*160/(4-v[2]),52-v[1]*160/(4-v[2])];
// Test projected faces with a small allowance for their drawn outline. This
// avoids canvas pixel readbacks and ignores the transparent animation padding.
export function dieContainsPoint(shape,angles,[x,y],padding=2){
 const vertices=shape.vertices.map(v=>projectVertex(rotateVertex(v,angles)));
 return shape.faces.some(ids=>{
  let positive=false,negative=false,nearEdge=false;
  for(let i=0;i<ids.length;i++){
   const [ax,ay]=vertices[ids[i]],[bx,by]=vertices[ids[(i+1)%ids.length]],dx=bx-ax,dy=by-ay,cross=dx*(y-ay)-dy*(x-ax),length2=dx*dx+dy*dy;
   if(cross>1e-6)positive=true;if(cross< -1e-6)negative=true;
   const t=length2?Math.max(0,Math.min(1,((x-ax)*dx+(y-ay)*dy)/length2)):0;
   if(Math.hypot(x-ax-t*dx,y-ay-t*dy)<=padding)nearEdge=true;
  }
