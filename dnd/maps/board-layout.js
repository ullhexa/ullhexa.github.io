...
export function createBoardLayout(){
  ...
  let open=false;const handle=button('›',()=>{open=!open;toolbar.classList.toggle('drawer-open',open);extra.inert=!open;handle.textContent=open?'‹':'›';handle.setAttribute('aria-expanded',String(open));handle.setAttribute('aria-label',open?'Hide additional map controls':'Show additional map controls');},'toolbar-drawer-handle');handle.setAttribute('aria-controls',extra.id);handle.setAttribute('aria-expanded','false');handle.setAttribute('aria-label','Show additional map controls');extra.inert=true;toolbar.replaceChildren(handle,extra,core);
  for(const[id,kind,label]of [['undo','undo','Undo'],['redo','redo','Redo'],['show-grid','grid','Grid'],['measure','ruler','Measure'],['mode-party','party','Party'],['mode-players','players','Players']]){const b=document.getElementById(id);b.replaceChildren(boardIcon(kind));b.setAttribute('aria-label',label);b.classList.add('board-icon-control');}
  ...
  function setExpanded(value){document.body.classList.toggle('map-expanded',value);expand.replaceChildren(boardIcon(value?'collapse':'expand'),el('span',value?'Restore panels':'Expand map'));expand.setAttribute('aria-pressed',String(value));}
  setExpanded(false);
}
