import {validCustomPlaces} from './custom-places.js?v=82';
import {IMAGE_ACCEPT,imageTypeNote,validImageDimensions} from './image-import.js?v=62';
import {readSessionValue,writeSessionValue} from './session-storage.js?v=62';
import {uploadImage,assetURL,assetRecord,putAssets} from './local-assets.js?v=79';
import {el} from './editor-dom.js?v=62';
import {editMapGrid} from './grid-editor.js?v=82';
import {assetId,safeId} from './combat-state.js?v=79';
...
export function createMapUpload({sessionKey,catalog,onAdded,announce}){
  ...
  upload.addEventListener('input',async()=>{const file=upload.files?.[0];if(!file||busy)return;busy=true;error.textContent='';try{
    ...
    const id=`custom-${crypto.randomUUID()}`,map={schemaVersion:1,userMap:true,id,version:'1',title:edit.title,width:prepared.width,height:prepared.height,places:edit.places,grid:edit.grid,art:{base:prepared.id,roofs:prepared.id},partyStart:[.5,.5],places:edit.places,interactions:[]};
...
export async function editCustomMap(entry){
  ...
  const edit=await editMapGrid(asset,entry.title,entry.map.grid,entry.map.places);if(!edit)return null;
  const prepared=edit.asset||asset;if(edit.asset)await putAssets([prepared]);
  return {...entry,title:edit.title,identity:edit.title,thumbnail:prepared.id,map:{...entry.map,title:edit.title,width:prepared.width,height:prepared.height,places:edit.places,grid:edit.grid,art:{base:prepared.id,roofs:prepared.id}}};
}
export const mapContentKey=map=>map.userMap?JSON.stringify([map.art.base,map.width,map.height,map.grid,map.places]):'';
