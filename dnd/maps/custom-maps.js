import {readSessionValue,writeSessionValue} from './session-storage.js?v=31';
import {uploadImage,assetURL,assetRecord,putAssets} from './local-assets.js?v=31';
import {el} from './editor-dom.js?v=31';
import {editMapGrid} from './grid-editor.js?v=31';
import {assetId,safeId} from './combat-state.js?v=31';
...
export function createMapUpload({sessionKey,catalog,onAdded,announce}){
  const upload=el('input');upload.type='file';upload.accept='image/png,image/jpeg,image/webp';upload.hidden=true;upload.setAttribute('aria-label','Upload battle map');const panel=document.getElementById('map-dialog'),error=el('p','','save-error');error.setAttribute('role','alert');panel.querySelector('.source-tabs').after(error);panel.append(upload);
  let busy=false;
