...
export function createDiceTools(preferences){
  ...
  function add(sides){
    if(pool.filter(d=>d.sides===sides).length>=20)return;const value=dieValue(sides),bank=diceFaceBank(sides),die={sides,value,parts:[]},group=button('',e=>e.detail===0||hitsDie(die,e)?remove(die):reset(),`dice-result${sides===100?' percentile-pair':''}`);die.group=group;group.style.width=`${sides===100?208:104}px`;group.style.flex='0 0 auto';group.dataset.sides=sides;group.dataset.value=value;group.setAttribute('aria-label',`Remove D${sides}: rolling`);group.addEventListener('pointermove',e=>group.classList.toggle('die-hovered',hitsDie(die,e)));group.addEventListener('pointerleave',()=>group.classList.remove('die-hovered'));
