...
import {createControlPreferences} from './control-preferences.js?v=82';
...
import {createGridControls} from './grid-controls.js?v=112';
import {gridColor} from './grid-state.js?v=112';
...
import {createUserManual} from './user-manual.js?v=112';
...
import {createItemsUI} from './items-ui.js?v=100';
...
import {createCombatUI,createLibraries} from './combat-ui.js?v=100';
...
import {createSessionBundle} from './session-bundle.js?v=112';
import { startDMShell } from './dm-shell.js?v=62';
import { openPlayerWindow } from './display-window.js?v=62';
import { validateMap, initialState, sanitizeState, isVisible, toggleInteraction, distanceBetween } from './state.js?v=112';
import { createEncounterTools } from './encounter-tools.js?v=106';
...
import { createSaveControls } from './save-controls.js?v=112';
import { parseSave, restoreSave } from './save-file.js?v=112';
import { createLighting } from './lighting.js?v=112';
...
async function start() {
  ...
  const send = (value,publicSnapshot=false) => { if(!publicSnapshot&&privatePreview&&['camera','positions','fog-preview','ruler','area-preview'].includes(value.type))return;if(channel)channel.postMessage(value);else writeStored(`${sessionKey}:signal`, { ...value, nonce: crypto.randomUUID() }); };
  ...
  const party = svgNode('g', { class: 'party-token', ...(player ? {} : { role: 'button', tabindex: 0, 'aria-label': 'Party marker. Drag to move; double-click to show players.' }) });
  ...
  party.append(svgNode('text', { 'text-anchor': 'middle', 'dominant-baseline': 'central', y: 0, fill: '#fff9dc', 'font-size': 28*partyUnit, 'font-weight': 700 }, 'P'));
  ...
  let positionSequence=0,lastPositionSequence=-1,areaSequence=0,lastAreaSequence=-1;
  ...
  let references;
  const encounter=createEncounterTools({map,player,getState:()=>state,commit,pointAt,announce,preview,finishDrag,getTool:()=>tool,setTool,livePositions,sendAreaPreview:area=>send({type:'area-preview',mapId:map.id,revision:state.revision,sequence:++areaSequence,area}),autoMeasure:()=>controls.get('autoMeasure'),measureMove:points=>{ruler=points;renderRuler(true);}});
  ...
  function publish(){
    ...
    if(!privatePreview)encounter.broadcastArea();
  ...
  function renderControls(cameraOnly=false) {
    if(!cameraOnly){
      const visible=placeMarkersVisible(state),layer=$('dm-hotspots');layer.style.display=player&&!visible?'none':'';layer.classList.toggle('place-markers-public',visible);layer.setAttribute('aria-hidden',String(player&&!visible));
      if(!player)$('show-place-markers').checked=visible;
      for(const place of map.places){
        const isSelected=!player&&place.id===selected;
        if(!player){placeButtons.get(place.id)?.setAttribute('aria-pressed',String(isSelected));floorControls.get(place.id)?.render();}
        const node=hotspots.get(place.id);if(node){node.classList.toggle('is-selected',isSelected);node.querySelector('circle').setAttribute('fill',isSelected?'#e8ba71':'#172a21');const name=placeName(place,state),tag=node.querySelector('.place-name-tag');if(tag.lastElementChild.textContent!==name){tag.lastElementChild.textContent=name;delete tag._width;}const row=placeButtons.get(place.id)?.firstElementChild;if(row?.lastChild)row.lastChild.textContent=name;node.setAttribute('aria-label',player?`${map.places.indexOf(place)+1}. ${name}`:`${name}. Click for quick actions; double-click to prepare building.`);}
      }
    }
    sizeHotspots();
    if(player)return;
    ...
    const focus=place?buildingFocusCamera(map,place,viewportSize(),controls.getZoom().maximum):null;
  ...
  function focusBuilding(place){
    ...
    state={...state,revision:state.revision+1};cameraAnimation.start(overview?{x:.5,y:.5,zoom:1}:buildingFocusCamera(map,place,viewportSize(),controls.getZoom().maximum));renderControls(true);
  }
  ...
  function render() {
    ...
    $('map-grid').firstElementChild.setAttribute('stroke-opacity',state.gridOpacity);
    gridControls?.render();
  ...
  function zoomBy(factor, anchor, immediate=false) {
    ...
    const zoom = clamp(before.zoom * factor, 1, controls.getZoom().maximum);
  ...
  function sizeHotspots(){
    if(player&&!placeMarkersVisible(state))return;
  ...
  function makeHotspot(place,point,number){
    const node=svgNode('g',{class:'hotspot','data-place':place.id,...(player?{role:'img','pointer-events':'none'}:{role:'button',tabindex:0})});
    ...
    glyph.append(svgNode('text',{class:'place-number-label','text-anchor':'middle',y:6,'font-size':17,'pointer-events':'none'},number));
  ...
  if (!player) {
    ...
    setupSidebarResize();
    $('zoom-in').addEventListener('click', () => zoomBy(1+controls.getZoom().step/100));
    $('zoom-out').addEventListener('click', () => zoomBy(1/(1+controls.getZoom().step/100)));
    ...
    $('open-player').addEventListener('click', () => {
      ...
      save();const url=new URL(location.href);url.search=new URLSearchParams({view:'player',session,map:map.id,build:'79',popup:'1'}).toString();
    ...
    $('map-stage').addEventListener('wheel', event => {if(event.target.closest('.token-status-editor,.shape-palette,.item-comment,.dice-panel,.reference-suite,.building-popup')||encounter.isDragging()||fog.isDrawing())return;event.preventDefault();wheelDelta+=event.deltaY;wheelPoint={clientX:event.clientX,clientY:event.clientY};if(!wheelFrame)wheelFrame=requestAnimationFrame(()=>{wheelFrame=0;const delta=wheelDelta;wheelDelta=0;zoomBy(Math.exp(-delta*.0015*controls.getZoom().wheel/100),pointAt(wheelPoint),true);});}, { passive: false });
  ...
  function receive(message) {
    ...
    if(player&&message.type==='area-preview'&&message.mapId===map.id&&message.revision===state.revision&&Number.isSafeInteger(message.sequence)&&message.sequence>lastAreaSequence){
      lastAreaSequence=message.sequence;const next=applyAreaPreview(state,message.area);state=next.state;encounter.receiveAreaPreview(next);
    }
    ...
    if (player && message.type === 'state' && message.state?.mapId === map.id && message.state?.mapVersion === map.version) {
      ...
      if(incoming.revision>=state.revision){const previousCamera=state.camera;cameraAnimation.cancel();incoming.camera=receiveCamera(incoming.camera,message.zoomPath);state=incoming;lastPositionSequence=-1;lastAreaSequence=-1;encounter.receiveAreaPreview();if(message.cameraDuration===2000){state={...state,camera:previousCamera};render();cameraAnimation.start(incoming.camera);}else render();}
  ...
  if(!player){createUserManual();director=createDirector({catalog,mapId:map.id,getProject:()=>project,setProject,prepareMap,announce});createLibraries({map,getState:()=>state,commit,announce});itemsUI=createItemsUI({map,getState:()=>state,commit,announce,pointAt,setTool,selectItem:id=>encounter.selectItem(id),editItem:id=>{references?.close();encounter.editItem(id);},showNotes:id=>{references?.close();encounter.showItemNotes(id);}});createSpellLibrary({getState:()=>state,commit,announce});references=createReferenceViewers({getState:()=>state,commit,announce,prepare:()=>{dice?.close();setTool(null);}});createBoardLayout(controls);createZoomControls({preferences:controls,onChange:()=>zoomBy(1)});const auto=document.getElementById('auto-measure');auto.setAttribute('aria-pressed',String(controls.get('autoMeasure')));document.querySelector('.map-controls').addEventListener('click',event=>{if(!event.target.closest('#measure')&&tool==='measure')setTool(null);},true);mapMenu.setUpload(createMapUpload({sessionKey,catalog,onAdded:entry=>{mapMenu.addEntry(entry);mapMenu.showEntry(entry);},announce}));}
  if(embedded){
    let lastActivity=0;
  ...
  runtimeReady=true;if(!player){save();requestAnimationFrame(()=>document.querySelector('.map-controls').classList.remove('controls-pending'));}
