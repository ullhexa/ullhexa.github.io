...
export function createBuildingControls({map,getState,getSelected,rename,getFocused,select,focus,toggle,setFloor,beginPreview,endPreview,prepare,getGeometry}){
 ...
 function updateButton(b,place){
  ...
  const available=kind==='focus'||!!(kind==='cover'?parts.roof:parts.collapsed);b.disabled=!available;b.setAttribute('aria-pressed',String(available&&(kind==='cover'?!on:on)));b.setAttribute('aria-label',`${placeName(place,getState())}: ${label}`);const key=`${kind}:${on}:${available}:${label}`;if(b.dataset.renderKey===key)return;b.dataset.renderKey=key;b.replaceChildren(symbol(kind,on));if(!b.classList.contains('icon-only')){const text=document.createElement('span');text.textContent=label;b.append(text);}
 }
 function populateEffects(parent,place){
  parent.replaceChildren();for(const item of buildingParts(map,place).effects){const b=document.createElement('button');b.type='button';b.className='building-effect';b.dataset.effect=item.id;b.setAttribute('aria-label',`${placeName(place,getState())}: ${item.label}`);const label=document.createElement('span');label.textContent=item.label;const levels=document.createElement('span');levels.className='effect-levels';for(const level of effectLevels(place,item)){const badge=document.createElement('span');badge.className='effect-floor';badge.textContent=level;badge.setAttribute('aria-label',`Floor ${level}`);levels.append(badge);}b.append(label,levels);b.addEventListener('click',()=>toggle(item.id));parent.append(b);}
 }
