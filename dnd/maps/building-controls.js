import {placeName} from './board-state.js?v=60';
import {buildingParts,effectLevels} from './building-state.js?v=60';
import {createFloorControl} from './floor-controls.js?v=60';
import {selectedFloor} from './floors.js?v=60';
...
export function createBuildingControls({map,getState,getSelected,rename,getFocused,select,focus,toggle,setFloor,beginPreview,endPreview,prepare,getGeometry}){
 ...
 function updateButton(b,place){
  ...
  const available=kind==='focus'||!!(kind==='cover'?parts.roof:parts.collapsed);b.disabled=!available;b.setAttribute('aria-pressed',String(kind==='cover'?!on:on));b.setAttribute('aria-label',`${placeName(place,getState())}: ${label}`);b.title=available?(kind==='collapse'?(on?'Restore intact building':'Collapse building'):label):'Not available for this place';b.replaceChildren(symbol(kind,on));if(!b.classList.contains('icon-only')){const text=document.createElement('span');text.textContent=label;b.append(text);}
 }
