...
export function createBuildingControls({map,getState,getSelected,rename,getFocused,select,focus,toggle,setFloor,beginPreview,endPreview,prepare,getGeometry}){
 ...
 function updateButton(b,place){
  ...
  const available=kind==='focus'||!!(kind==='cover'?parts.roof:parts.collapsed);b.disabled=!available;b.setAttribute('aria-pressed',String(kind==='cover'?!on:on));b.setAttribute('aria-label',`${placeName(place,getState())}: ${label}`);b.replaceChildren(symbol(kind,on));if(!b.classList.contains('icon-only')){const text=document.createElement('span');text.textContent=label;b.append(text);}
 }
