import {boundedCamera} from './camera.js?v=82';

// Fit the complete footprint into 60% of the stage, leaving context around it.
export function buildingFocusCamera(map,place,viewport,maximum=3){
 ...
 if(!shape?.length)return boundedCamera(map,{x:place.point[0],y:place.point[1],zoom:Math.min(maximum,(place.focusZoom||2)*.6)},viewport);
 ...
 const zoom=Math.min(maximum,.6*Math.min(viewport[0]/((right-left)*map.width*scale),viewport[1]/((bottom-top)*map.height*scale)));
