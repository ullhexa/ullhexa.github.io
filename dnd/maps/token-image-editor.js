import {showDialog} from './dialogs.js?v=37';
import {readRaster,rasterRecord,imageTypeNote,TOKEN_SIZE,TOKEN_SOURCE_SIZE} from './image-import.js?v=37';
import {el,button,label} from './editor-dom.js?v=37';
import {putAssets,assetRecord} from './local-assets.js?v=37';
...
export async function editTokenImage(input,{square=false,story=false,editing=false,title=''}={}){
  ...
  dialog.append(el('h2',story?'Edit story image':editing?'Edit token':'Create token'));if(story)dialog.append(label('Title',name));dialog.append(imageTypeNote(),wrap,label('Zoom',zoom),button('Fit',()=>{zoom.value=1;x=y=0;queue();}),error);
