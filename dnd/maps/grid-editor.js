...
export async function editMapGrid(asset,title,grid=null){
  ...
  return new Promise(resolve=>{
    ...
    function draw(){frame=0;c.fillStyle='#090c10';c.fillRect(0,0,960,540);const p=screen([0,0]),s=scale();c.drawImage(art,p[0],p[1],dimensions[0]*s,dimensions[1]*s);
      ...
      c.strokeStyle='#eec68c';c.lineWidth=2;c.strokeRect(p[0],p[1],dimensions[0]*s,dimensions[1]*s);for(const [x,y] of corners()){c.fillStyle='#15191e';c.fillRect(x-6,y-6,12,12);c.strokeStyle='#f4d09c';c.strokeRect(x-6,y-6,12,12);}
    }
    ...
    for(const [kind,text,name]of [['left','↶ 90°','Rotate map left 90 degrees'],['right','↷ 90°','Rotate map right 90 degrees'],['x','Mirror X','Mirror map horizontally'],['y','Mirror Y','Mirror map vertically']]){const b=button(text,()=>orient(kind));b.setAttribute('aria-label',name);b.title=name;orientation.append(b);}
