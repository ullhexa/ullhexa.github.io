...
export const FOG_TEXTURES=[
  {id:'clouds',name:'Clouds',url:'./assets/fog-of-war.png'},
  {id:'mist',name:'Silver mist',url:'./assets/fog-mist.webp'},
  {id:'storm',name:'Storm clouds',url:'./assets/fog-storm.webp'},
  {id:'haze',name:'Dust haze',url:'./assets/fog-haze.webp'}
];
...
export const validFogImage=i=>!!i&&fogAssetId(i.id)&&typeof i.name==='string'&&i.name.trim().length>0&&i.name.length<=80;
...
export const normalizeFogImages=value=>(Array.isArray(value)?value:[]).filter(validFogImage).filter((v,i,a)=>a.findIndex(n=>n.id===v.id)===i).slice(0,40).map(({id,name})=>({id,name}));
