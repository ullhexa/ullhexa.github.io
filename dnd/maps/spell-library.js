...
import {editUserSpell} from './spell-editor.js?v=97';
...
import {openCardFullscreen,setExpandButton,fullscreenButton} from './card-view.js?v=94';
...
import {loadSpells} from './spell-catalog.js?v=97';
...
export function createSpellLibrary({getState,commit,announce}){
 ...
 function render(){
  ...
  function renderApplied(){
   const spells=findSpells(catalog,'',currentDeck()?.spells||[]);print.disabled=!spells.length;applied.replaceChildren();
   for(const spell of spells){const chip=el('div',undefined,'applied-spell'),view=button(spellLabel(spell),()=>showSpellPages(spell)),del=button('×',()=>toggleSpell(spell.id));del.setAttribute('aria-label',`Remove ${spell.title} from deck`);chip.append(view,del);applied.append(chip);}
  }
