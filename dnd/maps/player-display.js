import {customCatalog} from './custom-maps.js?v=24';
import { setupFullscreen } from './fullscreen.js?v=24';
import { STORY_SCENES, createStoryAnimation } from './story-scenes.js?v=24';
import { validPresentation, sceneCanShow } from './presentation-state.js?v=24';
...
export async function startPlayerDisplay(){
  ...
  const response=await fetch('./maps/catalog.json?v=24');if(!response.ok)throw new Error('The map library could not load. Reload to try again.');
  ...
  function prepare(){
    ...
    const url=new URL(location.href);url.search=new URLSearchParams({view:'player',scene:'1',session,map:presentation.mapId,build:'24'}).toString();frame.src=url;
