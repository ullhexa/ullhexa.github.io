import {customCatalog} from './custom-maps.js?v=23';
import { setupFullscreen } from './fullscreen.js?v=23';
import { STORY_SCENES, createStoryAnimation } from './story-scenes.js?v=23';
import { validPresentation, sceneCanShow } from './presentation-state.js?v=23';
...
export async function startPlayerDisplay(){
  ...
  const response=await fetch('./maps/catalog.json?v=23');if(!response.ok)throw new Error('The map library could not load. Reload to try again.');
  ...
  function prepare(){
    ...
    const url=new URL(location.href);url.search=new URLSearchParams({view:'player',scene:'1',session,map:presentation.mapId,build:'23'}).toString();frame.src=url;
