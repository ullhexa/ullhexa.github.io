import {normalizeFeatures,featureEnabled,normalizeAura,armorClass} from './board-state.js?v=60';
import {normalizeFogImages} from './fog-state.js?v=60';
import {normalizeSpells,normalizeSpellLibrary} from './spell-state.js?v=60';
import {normalizeUserTokens} from './library-assets.js?v=60';
import {elevation} from './token-options.js?v=60';
...
export function normalizeMember(p,i=0,monster=false){
  ...
  return {aura:normalizeAura(p.aura),spells:normalizeSpells(p.spells),id:safeId(p.id)?p.id:`${monster?'monster':'player'}-${i+1}`,name:typeof p.name==='string'?p.name.trim().slice(0,32)||'Unnamed':'Unnamed',portrait:Number.isInteger(p.portrait)?clamp(p.portrait,0,monster?49:79):0,elevation:elevation(p.elevation),position:point(p.position)?[...p.position]:[.5,.5],avatar:assetId(p.avatar)?p.avatar:null,stack:Number.isSafeInteger(p.stack)&&p.stack>=0?p.stack:0,initiative:parseInitiative(p.initiative),conditions:[...new Set((Array.isArray(p.conditions)?p.conditions:[]).filter(id=>CONDITIONS.some(c=>c[0]===id)))],letters:[...new Set((Array.isArray(p.letters)?p.letters:[]).filter(s=>typeof s==='string'&&/^[A-Z]$/.test(s)))],...(monster?{monster:true,ac:armorClass(p.ac),tempHp:Math.max(0,Math.min(100000,Math.round(Number(p.tempHp)||0))),size:fiveFeet(p.size),visible:p.visible===true,maxHp:hp,hpLinked:p.hpLinked!==false,hp:Number.isFinite(p.hp)?clamp(Math.round(p.hp),0,hp):hp,statCard:assetId(p.statCard)?p.statCard:null,statText:typeof p.statText==='string'?p.statText.slice(0,20000):''}:{size:5})};
}
...
export function combatants(state){return [...(featureEnabled(state,'party')?state.roster:[]),...(featureEnabled(state,'encounter')?state.monsters||[]:[])];}
...
export function changeHP(member,amount,heal=false){const value=Math.max(0,Math.round(Number(amount)||0)),shield=member.tempHp||0;if(heal)return {...member,hp:clamp(member.hp+value,0,member.maxHp)};return {...member,tempHp:Math.max(0,shield-value),hp:clamp(member.hp-Math.max(0,value-shield),0,member.maxHp)};}
export function setTemporaryHP(member,amount){return {...member,tempHp:clamp(Math.round(Number(amount)||0),0,100000)};}
...
export function mapTokens(state){return [...combatants(state),...(featureEnabled(state,'items')?state.items||[]:[])];}
