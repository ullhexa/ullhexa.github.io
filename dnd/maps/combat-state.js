...
export function normalizeMember(p,i=0,monster=false){
  ...
  return {id:safeId(p.id)?p.id:`${monster?'monster':'player'}-${i+1}`,name:typeof p.name==='string'?p.name.trim().slice(0,32)||'Unnamed':'Unnamed',portrait:Number.isInteger(p.portrait)?clamp(p.portrait,0,monster?49:79):0,position:point(p.position)?[...p.position]:[.5,.5],avatar:assetId(p.avatar)?p.avatar:null,stack:Number.isSafeInteger(p.stack)&&p.stack>=0?p.stack:0,initiative:parseInitiative(p.initiative),conditions:[...new Set((Array.isArray(p.conditions)?p.conditions:[]).filter(id=>CONDITIONS.some(c=>c[0]===id)))],letters:[...new Set((Array.isArray(p.letters)?p.letters:[]).filter(s=>typeof s==='string'&&/^[A-Z]$/.test(s)))],...(monster?{monster:true,size:fiveFeet(p.size),visible:p.visible===true,maxHp:hp,hp:Number.isFinite(p.hp)?clamp(Math.round(p.hp),0,hp):hp,statCard:assetId(p.statCard)?p.statCard:null}:{size:5})};
}
...
export function normalizeItem(p,i=0){return {id:safeId(p.id)?p.id:`item-${i+1}`,item:true,templateId:safeId(p.templateId)?p.templateId:null,comment:typeof p.comment==='string'?p.comment.slice(0,2000):'',name:typeof p.name==='string'?p.name.trim().slice(0,32)||'Item':'Item',portrait:Number.isInteger(p.portrait)?clamp(p.portrait,0,499):0,avatar:assetId(p.avatar)?p.avatar:null,size:clamp(Math.round(Number(p.size)||5),1,200),position:point(p.position)?[...p.position]:[.5,.5],visible:p.visible===true,stack:Number.isSafeInteger(p.stack)&&p.stack>=0?p.stack:0};}
...
export function placeItem(state,template,id,position){if(!safeId(id)||mapTokens(state).some(m=>m.id===id)||!point(position)||(state.items||[]).length>=500)return state;return {...state,itemSchema:2,items:[...(state.items||[]),normalizeItem({...template,id,templateId:template.id,position,visible:false,stack:Math.max(0,...mapTokens(state).map(m=>m.stack||0))+1})]};}
