...
export function normalizeItem(p,i=0){return {id:safeId(p.id)?p.id:`item-${i+1}`,item:true,name:typeof p.name==='string'?p.name.trim().slice(0,32)||'Item':'Item',portrait:Number.isInteger(p.portrait)?clamp(p.portrait,0,99):0,avatar:assetId(p.avatar)?p.avatar:null,size:clamp(Math.round(Number(p.size)||5),1,200),position:point(p.position)?[...p.position]:[.5,.5],visible:p.visible===true,stack:Number.isSafeInteger(p.stack)&&p.stack>=0?p.stack:0};}
export function normalizeItems(list){const seen=new Set();return (Array.isArray(list)?list:[]).slice(0,100).filter(p=>p&&safeId(p.id)&&!seen.has(p.id)&&seen.add(p.id)).map(normalizeItem);}
